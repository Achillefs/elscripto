...
module Elscripto
  class Command
    ...
    def initialize(name, options = {})
      raise ArgumentError, 'Elscripto commands need a name spectacularrr' if blank? name

      @name = name
      @system_call = blank?(options[:command]) ? builtin_commands[name] : options.delete(:command)
      raise ArgumentError, 'Elscripto commands need a command spectacularrr' if blank? @system_call
    end
